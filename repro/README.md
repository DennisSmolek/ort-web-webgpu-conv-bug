...
The ONNX model is correct: it matches Intel's native OIDN to 54 dB on the
CPU/WASM provider (verified separately against the reference implementation).
The model file is not at fault — only the WebGPU EP diverges.

## Isolation: it is the *graph-input* conv specifically, not any wide conv

A second test (`../split`) narrows the fault precisely. The OIDN
...
- **GPU-side workaround — VERIFIED (not just hypothesised):** compute the one
  bad conv (`enc_conv0`) off the WebGPU EP (in a WGSL kernel, or on WASM),
  re-export the model to start at `enc_conv1` (two inputs:
